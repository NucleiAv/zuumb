...
- **Never wire the response layer to actually execute anything through Phases 1-11** (no real firewall/EDR API calls) — keep it "propose + human click to mark done." This is a safety and scope boundary for the POC/MVP, not just a nice-to-have. Real execution is deliberately deferred to Section 11 (post-MVP), gated behind Wazuh's own Active Response module and still human-approval-only, never auto-fire.
...
## 11. Post-MVP: Live Wazuh, Real Response, Public Distribution
...
|---|---|---|
| `WAZUH_API_USER` / `WAZUH_API_PASSWORD` (ingestion) | Reading alerts via `GET /alerts` or indexer query | Read-only on alerts/events. This is what `config.py` already has fields for. |
| `WAZUH_AR_API_USER` / `WAZUH_AR_API_PASSWORD` (active response, Phase 14 only) | Dispatching `PUT /active-response` commands | Write access scoped to active-response only, ideally restricted to specific agent groups, not a full-admin account. Never reuse the ingestion credential for this. |

Confirm both exist as separate users in your Wazuh instance's RBAC config before Phase 14. Using one over-privileged credential for everything is a real security smell in a project whose whole pitch is "AI-native detection and response" — worth doing this correctly.

### Phase 12 — Live Wazuh Integration

*Persona: Backend Architect.*

Goal: replace synthetic-file ingestion with real polling against a live Wazuh instance, no other pipeline stage changes.

1. Wire `config.py`'s existing (currently unused) `wazuh_api_url` / `wazuh_api_user` / `wazuh_api_password` / `verify_ssl` fields into `wazuh_client.py`'s actual HTTP calls.
2. Implement scheduled polling (APScheduler, already in the stack) against the real alerts endpoint, on a configurable interval (start at 60s, tune later).
3. Before wiring anything else downstream, fetch a batch of real alert responses and diff their shape against the normalizer built in Phase 2. Beta/version drift in field names or structure is the most likely real failure here — confirm this explicitly, don't assume the documented schema matches your running version.
4. Track a `last_polled_id`/timestamp cursor so re-polling doesn't re-ingest the same alerts (the idempotency already verified in the Phase 4 POC run should extend cleanly here, but verify against live data too).
5. Everything downstream (triage, correlation, chains, dashboard) should need zero changes, if it does, that's a signal the ingestion normalizer isn't actually producing the same shape as synthetic data did, fix at the normalizer, not by forking logic downstream.

Checkpoint: run zuumb against your live Wazuh instance for at least an hour of real traffic, confirm triage/correlation/chains all populate correctly from real data. Run `/ponytail-audit`. Stop and show results.

### Phase 13 — Real Attack Chains at Scale

*Persona: Backend Architect, then Reality Checker.*

Mostly validation, not new code, since the stitcher is already data-source agnostic.

1. Let live data accumulate for a few days, confirm chains are forming from genuinely diverse real traffic, not just the synthetic patterns you designed.
2. Tune the correlation time-window (currently 30 min) against real traffic density, if your environment is noisier than the synthetic set, the window may need shrinking to avoid over-merging unrelated alerts into one incident.
3. Watch specifically for false-chain formation (two unrelated incidents getting stitched together just because they share a common host like a proxy or jump box), this is the main new failure mode live traffic introduces that synthetic data didn't have.

Checkpoint: `/ponytail-review`, then confirm with Reality Checker persona that chain quality holds up against a week of real traffic before moving to Phase 14.

### Phase 14 — Human-Approved Active Response (Real Execution)

*Persona: Security Engineer leads this phase, Backend Architect implements.*

This is the phase that changes the Section 8 guardrail. Real execution is now in scope, but the human-approval gate is non-negotiable and gets stricter, not looser.

1. **Build on Wazuh's Active Response module, don't build custom remote execution.** Dispatch via `PUT /active-response` to Wazuh's manager, targeting existing hardened scripts (`firewall-drop` for IP blocking, `host-deny`, or a custom-but-Wazuh-native script you register on the agent) rather than any custom SSH/WinRM/agent-side execution path zuumb would have to secure itself.
2. **Scope the action set to a fixed, reviewed allowlist**, not arbitrary commands. Start with 2-3 well-understood actions: block source IP, disable/lock user account, isolate host from network (via existing Wazuh AR scripts or documented custom ones). Do not add "run arbitrary script" as an option.
3. **Approval flow, updated from the MVP's no-op version:**
   - Analyst clicks Approve on a task exactly as before
   - Now, on approve, the backend calls the Wazuh AR API with the specific scoped action against the specific agent, using the `WAZUH_AR_API_USER` credential
   - Log the full request/response (what was dispatched, to which agent, when, by whom) to an audit table, `response_actions_log` — this is not optional, an unauditable auto-mitigation system is a liability, not a feature
   - Surface the actual execution result (success/failure) back in the UI, not just "task marked done"
4. **Add a dry-run/simulation mode as a config flag**, defaulting to on, so you (or anyone installing this publicly) can test the full approval flow without it touching a real host until they deliberately flip it off. This matters a lot for anyone who pulls the public Docker image and doesn't fully understand what flipping this on does.
5. **Rate-limit and confirm-twice for destructive actions** (host isolation especially), a single misclick isolating a production host is a real operational risk worth designing against, a second confirmation step for the highest-impact action types is cheap insurance.
6. Update `tests/test_response.py`'s execution-primitive guard test, it currently fails the build if execution primitives appear in `app/response/playbooks.py` at all. This needs to become a scoped allowlist test instead (only the specific Wazuh AR API call path is permitted, nothing else), not simply deleted, the point of that test was never "no execution ever," it was "no *unreviewed* execution."

Checkpoint: test the full flow end to end against a real (non-production, throwaway) test agent, confirm dry-run mode works, confirm the audit log captures everything, confirm a real action executes and is visible on the target host. `/ponytail-audit`. Do not skip the throwaway-agent testing step, this phase is the one place in the whole project where a bug has real-world consequences beyond a wrong dashboard number.

### Phase 15 — Public Docker Distribution

*Persona: Backend Architect for the pipeline, Product for the README/quickstart framing.*

1. Multi-stage production Dockerfile for the app service (confirm no dev-only bind mounts or debug flags leak into the final image).
2. GitHub Actions workflow (`.github/workflows/publish.yml`): builds on version tags (`v*.*.*`), pushes multi-arch (amd64 + arm64) images to `ghcr.io/<username>/zuumb` tagged both by version and `latest`, using `GITHUB_TOKEN`, no extra secrets needed.
3. Two compose files: `docker-compose.yml` (end users, pulls the published image) and `docker-compose.dev.yml` (contributors, builds from local source).
4. **Explicitly set the GHCR package visibility to public** after first push, new packages default to private even on a public repo, this silently breaks installation for anyone who isn't you if missed.
5. README quickstart: `git clone` → `cp .env.example .env` (both credential pairs from the table above, plus `ANTHROPIC_API_KEY`) → `docker compose up`. Confirm this genuinely works from a clean machine with zero local build step.

...
# `alert_field` -> which `alert.data.*` key the target goes under; each AR script
# reads a different one (firewall-drop wants srcip, disable-account wants dstuser).
ACTIONS: dict[str, dict] = {
    "block-ip": {"command": "!firewall-drop", "confirm": False, "alert_field": "srcip"},
    "disable-user": {"command": "!disable-account", "confirm": True, "alert_field": "dstuser"},
}
...
def dispatch(action: str, target: str, agent_id: str, *,
    ...
    try:
        ...
        r = client.put(
            ...
            json={"command": spec["command"], "arguments": [target],
                  "alert": {"data": {spec["alert_field"]: target}}},
        )
